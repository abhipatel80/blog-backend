...
// create new blog
router.post("/add", auth, uploadfile.single("file"), async (req, res) => {
  try {
    let blogImage;
    if (req?.file) {
    ...
    if (description.split("").length > 2000) {
      return res
        .status(401)
        .json("description not be greater than 2000 characters");
    }
...
router.put("/comment", auth, async (req, res) => {
  try {
    ...
    const isCommented = blog.comments.find(
      (val) => val.userId.toString() === req.user._id.toString()
    );
    if (!comment || !blogId) {
...
router.get("/get", async (req, res) => {
  try {
    if (req.query.name) {
      const search = await blogModel.find({
...
// edit blog
router.put("/edit/:id", auth, uploadfile.single("file"), async (req, res) => {
  try {
    let blogImage;
    if (req?.file) {
